...
foreach ($kv in $expected.GetEnumerator()) {
    ...
    if (-not $found) {
        switch ($arcName) {
            "README.md" {
                @"# ArmyHub Officer Package $Version

Цей пакет містить ключові документи ArmyHub для офіцерів: Overview, Strategic Report, Tech Map, Roadmap, Implementation Guide.

## Як користуватися
1) Прочитайте *Officer_Package_Overview.md*.
2) Перегляньте *Roadmap_ARMY.png* та *Tech_Map_ARMY.png*.
3) Запустіть MVP за *Implementation_Guide.pdf*.

© $(Get-Date -Format yyyy) ArmyHub
"@ | Set-Content -Path $destPath -Encoding UTF8
                Write-Warn "Generated default README.md"
            }
            "CHANGELOG.md" {
...
"@ | Set-Content -Path $destPath -Encoding UTF8
                Write-Warn "Generated default CHANGELOG.md"
            }
            "MANIFEST.md" { 
            ...
            Default {
                Write-Warn "Missing optional: $arcName (no source file found)"
            }
        }
    }
}

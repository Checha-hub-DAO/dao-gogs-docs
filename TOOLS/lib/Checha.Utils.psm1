...
function Format-Trend { param([double]$curr, $prev)
...
function Parse-DoneSharePct { param([string]$s)
...
function Acquire-ChechaLock { param([string]$Path,[int]$TimeoutMinutes=5)
  $sw=[System.Diagnostics.Stopwatch]::StartNew()
  while($true){ try{ $fs=[System.IO.File]::Open($Path,'CreateNew','Write','None'); $fs.Close(); return }
...
function Backup-TextFile { param([Parameter(Mandatory)][string]$Path,[Parameter(Mandatory)][string]$Root)
